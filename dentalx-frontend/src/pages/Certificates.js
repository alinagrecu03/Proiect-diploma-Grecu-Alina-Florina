...
export function Certificates() {
    ...
    const documentList = useGetDocuments("certificates", userDetails?.userid);
    ...
    const documentMutation = useMutation((data) => writeFileToServer(data), {
        onSuccess: (response) => {
            queryClient.invalidateQueries(`${userDetails?.userid}-certificates-list`);
            ...
            <div className="document-template">
                {
                    ...
                    (    <div>
                            ...
                            <div>
                            {
                                expandedRow?.name && <a className="btn-fill p-2 no-underline" download={expandedRow?.name?.split("_certificates_")?.[0] ?? ""} href={`data:application/pdf;base64,${pdfString}`}>Download</a>
                            }
                            </div>
                        </div>
                    )
    ...
    const handleOnAddFile = (data) => {
        documentMutation.mutate({ id: data.patientid, file: data.documents, type: "certificates" });
        setOpenDialog(false);
    };
    ...
    const bodyActions = (dataRow) => {
        return (
            <Button label="Delete" onClick={ () => handleDeleteCertificate(dataRow) } />
        )
    };
    // functie pentru a afisa numele documentului pentru fiecare linie
    const bodyName = (dataRow) => {
        return dataRow?.name?.split("_certificates_")?.[0]?.replaceAll("_", " ");
    };
    ...
    return (
        <div className="Certificates">
            <Button label="Add Document" className='my-10' onClick={() => handleOnOpenDialog() } />
            ...
            >
                ...
                <Column field="lastname" header={`${userDetails?.account === "doctor" ? "Patient" : "Doctor"} Lastname`}></Column>
                {
                    userDetails?.account === "doctor" && (
                        <Column field="actions" header="Actions" body={ bodyActions }></Column>
                    )
                }
            </DataTable>
            <Dialog
                header={() => <h3>Add a document</h3>}
                visible={ openDialog }
